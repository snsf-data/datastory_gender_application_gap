...
datastory_name <- "gender_application_gap"
...
# characters etc.)
datastory_name_de <- "weniger-foerdergesuche-frauen"
datastory_name_en <- "women-submitting-fewer-grant-proposals"
datastory_name_fr <- "moins-demandes-financement-femmes"

# English title and lead of the story (Mandatory, even if no EN version)
title_en <- "Are women submitting fewer grant proposals?" 
lead_en <- "Studies and surveys confirm that during the COVID-19 pandemic, women’s workload at home has increased. Does that mean women researchers are also submitting fewer proposals to the SNSF? Analyses show that, with one exception, their share has remained stable. " # Mandatory, even if no lang version
# German title and lead of the story (Mandatory, even if no DE version)
title_de <- "Weniger Fördergesuche von Frauen?" 
lead_de <- "Frauen leisten während der Covid-19-Pandemie zu Hause Mehrarbeit. Dies bestätigen Studien und Umfragen. Haben Forscherinnen beim SNF weniger Gesuche eingereicht? Analysen zeigen: Mit einer Ausnahme ist ihr Anteil gleich geblieben." # Mandatory, even if no lang version
# French title and lead of the story (Mandatory, even if no FR version)
title_fr <- "Moins de demandes de financement de la part des femmes ?" 
lead_fr <- "Pendant la pandémie de Covid-19, les femmes ont vu leurs charges domestiques augmenter. Des études et des enquêtes le confirment. Les chercheuses ont-elles donc soumis moins de requêtes au FNS ? Selon les analyses, leur part est restée stable à une exception près." # Mandatory, even if no lang version
# Contact persons, always (first name + last name)
contact_person <- c("Simona Isler", "Laura Lots", "Rachel Heyard", 
                    "Julius Mattern", "Anne Jorstad")
...
publication_date <- "2021-05-04 07:00:00"
...
github_url <- "https://github.com/snsf-data/datastory_gender_application_gap" 
# Put Tag IDs here. Only choose already existing tags. 
tags_ids <- c(170, 60, 80, 90, 100)
